...
void TryPutSecretBaseSecretsOnAir(void)
{
    ...
    if (IsRecordMixShowAlreadySpawned(TVSHOW_SECRET_BASE_SECRETS, FALSE) != TRUE)
    {
        ...
        if (sCurTVShowSlot != -1)
        {
            ...
            if (show->secretBaseSecrets.language == LANGUAGE_JAPANESE || gSaveBlock1Ptr->secretBases[VarGet(VAR_GOT_STARTER)].language == LANGUAGE_JAPANESE)
                show->secretBaseSecrets.baseOwnersNameLanguage = LANGUAGE_JAPANESE;
            else
                show->secretBaseSecrets.baseOwnersNameLanguage = gSaveBlock1Ptr->secretBases[VarGet(VAR_GOT_STARTER)].language;
        }
    }
}
