...
static void Task_EnterSecretBase(u8 taskId)
{
    ...
    case 0:
        ...
        break;
    case 1:
        secretBaseIdx = VarGet(VAR_GOT_STARTER);
...
void InitSecretBaseDecorationSprites(void)
{
    ...
    {
        u16 secretBaseIdx = VarGet(VAR_GOT_STARTER);
        decorations = gSaveBlock1Ptr->secretBases[secretBaseIdx].decorations;
    ...
    for (i = 0; i < numDecorations; i++)
    {
        ...
        if (permission == DECORPERM_SPRITE)
        {
             ...
             || MetatileBehavior_HoldsLargeDecoration(metatileBehavior) == TRUE)
            {
                ...
                if (CurMapIsSecretBase() == TRUE && VarGet(VAR_GOT_STARTER) != 0)
                {
...
{
    VarSet(VAR_OBJ_GFX_ID_F, sSecretBaseOwnerGfxIds[GetSecretBaseOwnerType(VarGet(VAR_GOT_STARTER))]);
}
...
{
    return GetSecretBaseName(dest, VarGet(VAR_GOT_STARTER));
}
...
{
    if (IsSecretBaseRegistered(VarGet(VAR_GOT_STARTER)) == TRUE)
...
{
    gSaveBlock1Ptr->secretBases[VarGet(VAR_GOT_STARTER)].registryStatus ^= 1;
...
{
    u8 ownerType = GetSecretBaseOwnerType(VarGet(VAR_GOT_STARTER));
    if (ownerType == 0)
...
{
    gSaveBlock1Ptr->secretBases[VarGet(VAR_GOT_STARTER)].battledOwnerToday = gSpecialVar_Result;
}
