...
static const u16 sSpeciesToHoennPokedexNum[NUM_SPECIES - 1] =
{
    ...
    SPECIES_TO_SINNOH(MIMEJR),
    SPECIES_TO_SINNOH(MRMIME),
...
static const u16 sHoennToNationalOrder[Sinnoh_Dex_COUNT - 1] =
{
...
u16 NationalToHoennOrder(u16 nationalNum)
{
    ...
    while (hoennNum < (Sinnoh_Dex_COUNT - 1) && sHoennToNationalOrder[hoennNum] != nationalNum)
        hoennNum++;

    if (hoennNum >= Sinnoh_Dex_COUNT - 1)
        return 0;
...
{
    if (!hoennNum || hoennNum >= Sinnoh_Dex_COUNT)
        return 0;
...
{
    if (SpeciesToHoennPokedexNum(species) > Sinnoh_Dex_COUNT)
        return FALSE;
    else
        return TRUE;
}
